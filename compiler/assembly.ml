...
let rec i_to_asm (i : instruction) : string =
  ...
  | ICmovl (dest, value) -> sprintf "  cmovl %s, %s" (arg_to_asm dest) (arg_to_asm value)
  ...
  | ILabel (name, None) -> name ^ ":"
  | ILabel (name, Some n) -> sprintf "  align %d\n  %s:" n name
